...
class GameManager:
    def __init__(self):
        ...
        # UI Constants
        self.CELL_SIZE = 60  # Increased cell size
        self.GRID_OFFSET_X = 50
        self.GRID_OFFSET_Y = 50
        self.SPELL_HEIGHT = 120
        self.COLORS = {
            'background': (20, 20, 30),  # Darker blue-tinted background
            'grid': (40, 40, 60),
            'player': (0, 255, 200),  # Cyan-tinted green
            'enemy': (255, 50, 50),   # Brighter red
            'selected': (255, 255, 0),
            ...
            'ap_mp': (0, 255, 255),    # Cyan for AP/MP display
            'hp_bar_bg': (60, 60, 60),
    ...
    def setup_game(self, screen):
        ...
        # Create one player character
        self.player = Player("Hero", (1, 1))
        
        # Create three monsters
        self.monsters = [
    ...
    def handle_mouse_click(self, pos):
        ...
        if 0 <= grid_x < self.board.width and 0 <= grid_y < self.board.height:
            ...
            if current_char == self.player:  # Only handle clicks during player's turn
    ...
    def handle_key_press(self, key):
        current_char = self.turn_order[self.current_player_index]
        if current_char == self.player:  # Only handle key presses during player's turn
            if key == pygame.K_F1:  # Skip turn with F1
                self.end_turn()
            ...
            elif key == pygame.K_ESCAPE:  # Cancel selection
                self.selected_spell = None
                self.highlighted_cells.clear()
    ...
    def highlight_movement_range(self, character: Character):
        self.highlighted_cells = set(
            self.board.get_movable_positions(character.position, character.movement_points)
        )

    def highlight_spell_range(self, character: Character, spell: dict):
        self.highlighted_cells.clear()
        range_val = spell['range']
        for x in range(max(0, character.position[0] - range_val),
                      min(self.board.width, character.position[0] + range_val + 1)):
            for y in range(max(0, character.position[1] - range_val),
                         min(self.board.height, character.position[1] + range_val + 1)):
                if (x, y) != character.position:
                    distance = abs(x - character.position[0]) + abs(y - character.position[1])
    ...
    def cast_spell(self, character: Character, spell: dict, target_pos: Tuple[int, int]) -> bool:
    ...
    def move_character(self, character: Character, new_pos: Tuple[int, int]):
        if character.movement_points > 0:
            ...
            character.movement_points = 0  # Use all movement points when moving
        ...
        for x in range(self.board.width):
            for y in range(self.board.height):
                ...
                if (x, y) in self.highlighted_cells:
                    ...
                    color = self.COLORS['highlight_attack'] if self.selected_spell else self.COLORS['highlight_move']
    ...
    def draw_characters(self):
        for char in self.all_characters:
            ...
            color = self.COLORS['player'] if char.team == 'player' else self.COLORS['enemy']
    ...
    def draw_spell_panel(self):
        panel_rect = pygame.Rect(0, self.height - self.SPELL_HEIGHT,
                               self.width, self.SPELL_HEIGHT)
        pygame.draw.rect(self.screen, self.COLORS['spell_panel'], panel_rect)

        current_char = self.turn_order[self.current_player_index]
        if current_char == self.player:  # Only show spells for player
            x_offset = 10
            for i, (spell_name, spell) in enumerate(current_char.spells.items()):
                color = spell['color'] if spell_name == self.selected_spell else self.COLORS['text']
                text = f"{i+1}: {spell_name} (AP: {spell['ap_cost']})"
                text_surface = self.font.render(text, True, color)
                self.screen.blit(text_surface, (x_offset, self.height - self.SPELL_HEIGHT + 10))
    ...
    def draw_status_panel(self):
        ...
        for i, char in enumerate(self.turn_order):
            color = self.COLORS['current_turn'] if i == self.current_player_index else self.COLORS['text']
    ...
    def draw_game_over(self):
        ...
        restart_text = font_small.render("Press R to restart", True, self.COLORS['text'])
    ...
    def run_game(self):
        ...
        # Initial movement range highlight for player's turn
        ...
        while running:
            ...
            if not self.game_over:
                ...
                if current_char in self.monsters:
                    # Simple AI: move towards player and attack if in range
    ...
    def can_attack_player(self, monster: Character) -> bool:
        if not monster.spells:
            return False
        spell = list(monster.spells.values())[0]
        distance = abs(monster.position[0] - self.player.position[0]) + \
                  abs(monster.position[1] - self.player.position[1])
        return distance <= spell['range']

    def update(self):
        self.check_game_over()
        if self.game_over:
            # Reset game state or provide option to restart
            pass
